      ...
      {items.map((it, i) => {
        ...
        return (
          <Glass key={it.title} className="flex h-full flex-col p-7 md:p-8">
            ...
            <h3 className="mt-6 text-[19px] font-medium tracking-[-0.02em] text-[#010110] md:text-[20px]">
              {it.title}
            </h3>
    ...
    <div className="grid gap-5 md:grid-cols-3">
      {items.map((it, i) => (
        <Glass key={it.title} className="relative flex h-full flex-col overflow-hidden p-7 md:p-8">
          ...
          <h3 className="mt-4 text-[19px] font-medium tracking-[-0.02em] text-[#010110] md:text-[20px]">
            {it.title}
          </h3>
          <Body className="mt-3 text-[15px]">
            {it.desc}
...
export function CardsTags({ items }: { items: Item[] }) {
  return (
    <div className="grid gap-5 md:grid-cols-3">
      {items.map((it, i) => (
        <Glass key={it.title} className="flex h-full flex-col p-7 md:p-8">
          <h3 className="text-[19px] font-medium tracking-[-0.02em] text-[#010110] md:text-[20px]">
            {it.title}
          </h3>
          ...
          <ul className="mt-6 flex flex-wrap gap-2 border-t border-[#010110]/8 pt-5">
            {(tags[i] ?? []).map((t) => (
              <li
                key={t}
